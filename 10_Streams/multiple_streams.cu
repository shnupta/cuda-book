...
int main(void) {
  ...
  for (int i = 0; i < FULL_DATA_SIZE; i += 2 * N) {
    ...
    // Above doesn't utilise the streams because of the way the driver keeps intrastream dependencies remain satisfied by the hardware's execution units
    // Below we don't require the kernel of stream0 to run before the copy back to dev_c0, which originally prevented the copy from dev_a1 from running
    HANDLE_ERROR(cudaMemcpyAsync(dev_a0, host_a + i, N * sizeof(int), cudaMemcpyHostToDevice, stream0));
    HANDLE_ERROR(cudaMemcpyAsync(dev_a1, host_a + i + N, N * sizeof(int), cudaMemcpyHostToDevice, stream1));
